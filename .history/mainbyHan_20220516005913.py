...
draw_board(screen)
load_image()
black_win = pygame.transform.scale(
    pygame.image.load("images/bK.png"), (256, 256))
white_win = pygame.transform.scale(
    pygame.image.load("images/wK.png"), (256, 256))
selected_icon = pygame.transform.scale(
    pygame.image.load("images/selected.png"), (SQ_SIZE, SQ_SIZE))
mimimax_player = minimax_agent.MinimaxPlayer(human_chess_color)
...
def human(human_chess_color):
    ...
    while 1:
        ...
        if board.turn == human_chess_color:
            ...
            if isinstance(mimimax_player, random_agent.RandomPlayer):
                move = mimimax_player.get_move(board)
            else:
                move = mimimax_player.get_move(board, DEPTH)
            board.push(move)
            last_move = move
...
def middle_vs_minmax(minimax_chess_color):
    while 1:
