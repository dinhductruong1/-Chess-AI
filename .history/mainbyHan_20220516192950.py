                            ...
                            for move in list(board.legal_moves):
                                if move.from_square == first_square and move.to_square == second_square:
                                    ...
                                    if prueba.promotion is not None:
                                        #display promotion menu
                                        menu = pygame_menu.Menu(
                                            window_title='Promotion',
                                            window_width=400,
                                            window_height=300
                                        )
                                        menu.add_selector(
                                            'Promotion',
                                            [
                                                pygame_menu.widgets.Option('Queen', promotion_type='q'),
                                                pygame_menu.widgets.Option('Rook', promotion_type='r'),
                                                pygame_menu.widgets.Option('Bishop', promotion_type='b'),
                                                pygame_menu.widgets.Option('Knight', promotion_type='n')
                                            ],
                                            onchange=promotion_type
                                        )
                                        menu.add_button('OK', None)
                                        menu.mainloop(
                                            screen)
...
def start_the_game():
    global bot1, bot2, is_human, human_chess_color
    if is_human:
        print('danh cho nguoi choi')
...
def promotion(selected: Tuple, value: Any):
    global promotion_piece
    promotion_piece = value
...
promotion.add.button('Queen', promotion('q'))
promotion.add.button('Rook', promotion('r'))
promotion.add.button('Bishop', promotion('b'))
promotion.add.button('Knight', promotion('n'))
